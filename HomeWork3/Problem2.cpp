...
void ZeroWell::normalize(std::vector<double> inpsi) {
    std::vector<double> psipsi;
    double s;
    double dummy = inpsi.size() / 2;  // doing it like this in case I put an odd number of well points sometime in the future
    int n = (int)dummy;
    ...
    for (int i = 0; i < inpsi.size(); i++) {
        norm_psi.push_back(inpsi[i] / s);
    }
...
void HarmonicWell::normalize(std::vector<double> inpsi) {
    std::vector<double> psipsi;
    double s;
    double dummy = inpsi.size() / 2;  // doing it like this in case I put an odd number of well points sometime in the future
    int n = (int)dummy;
...
void HarmonicWell::write_function(fs::path outfile) {
    std::cout << "Writing to file:\n" << outfile << std::endl;
    // std::cout << "tmax = " << tmax << std::endl;
...
std::vector<double> AnharmonicWell::wavefunction(double e) {
    std::vector<double> psi_func;
    double psi, phi;
    double k_psi_1, k_psi_2, k_psi_3, k_psi_4;
    double k_phi_1, k_phi_2, k_phi_3, k_phi_4;
...
void AnharmonicWell::normalize(std::vector<double> inpsi) {
    std::vector<double> psipsi;
    double s;
    double dummy = inpsi.size() / 2;  // doing it like this in case I put an odd number of well points sometime in the future
    int n = (int)dummy;
...
void AnharmonicWell::write_function(fs::path outfile) {
    std::cout << "Writing to file:\n" << outfile << std::endl;
    // std::cout << "tmax = " << tmax << std::endl;
